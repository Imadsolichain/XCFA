...
export default function CerfaProAPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    toast.success('Cerfa Pro-A généré (mock) !');
    setForm({ nom: '', prenom: '', dateNaissance: '', siret: '' });
    setSubmitting(false);
  }
    ...
    <Layout title="Cerfa Pro-A">
      <div className="max-w-2xl mx-auto py-12">
        ...
        <form className="bg-white rounded-xl shadow p-6 mb-8 space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label className="block mb-1 font-medium">SIRET</label>
            <input className="border rounded w-full p-2" required value={form.siret} onChange={e => setForm(f => ({ ...f, siret: e.target.value }))} />
          </div>
        ...
        <h2 className="text-xl font-semibold mb-4 text-[#333]">Mes Cerfa Pro-A générés</h2>
          ...
          <ul className="divide-y divide-[#E9F0FF]">
            ...
            {cerfas.map((c) => (
              <li key={c.id} className="py-4 flex items-center justify-between">
                <div>
                  <span className="text-[#2F5FDE] font-semibold">Cerfa Pro-A</span>
